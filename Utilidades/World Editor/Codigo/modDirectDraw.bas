...
Function LoadWavetoDSBuffer(ByRef DS As DirectSound, _
                            ByRef DSB As DirectSoundBuffer, _
                            ByRef sfile As String) As Boolean
    '*************************************************
    'Author: Unkwown
...
Public Sub DDrawGrhtoSurface(ByRef Surface As DirectDrawSurface7, _
                             ByRef Grh As Grh, _
                             ByVal X As Integer, _
...
error:
    ...
    Else
        MsgBox "Ocurrió un error inesperado, por favor comuniquelo a los administradores del juego." & vbCrLf & _
                "Descripción del error: " & vbCrLf & Err.Description, vbExclamation, "[ " & Err.Number & " ] Error"
        End
...
Public Sub DDrawTransGrhIndextoSurface(ByRef Surface As DirectDrawSurface7, _
                                       ByVal GrhIndex As Long, _
                                       ByVal X As Integer, _
                                       ByVal y As Integer, _
...
Public Sub DDrawTransGrhtoSurface(ByRef Surface As DirectDrawSurface7, _
                                  ByRef Grh As Grh, _
                                  ByVal X As Integer, _
                                  ByVal y As Integer, _
                                  ByVal Center As Byte, _
    ...
    For y = screenminY To screenmaxY
        ...
        For X = screenminX To screenmaxX
            ...
            If MapData(X, y).Graphic(1).GrhIndex <> 0 Then
                Call DDrawGrhtoSurface(BackBufferSurface, MapData(X, y).Graphic(1), (ScreenX - 1) * TilePixelWidth + _
                        PixelOffsetX + TileBufferPixelOffsetX, (ScreenY - 1) * TilePixelHeight + PixelOffsetY + _
                        TileBufferPixelOffsetY, 0, 1)
                ...
                If CurLayer = 1 Then
                    If X = SobreX And y = SobreY Then
                        If frmConfigSup.MOSAICO.Value = vbChecked Then
                            Call DDrawGrhtoSurface(BackBufferSurface, CurrentGrh(((X + DespX) Mod mAncho) + 1, ((y + _
                                    DespY) Mod MAlto) + 1), (ScreenX - 1) * TilePixelWidth + PixelOffsetX + _
                                    TileBufferPixelOffsetX, (ScreenY - 1) * TilePixelHeight + PixelOffsetY + _
                                    TileBufferPixelOffsetY, 0, 1)
                        Else
                            Call DDrawGrhtoSurface(BackBufferSurface, CurrentGrh(0), (ScreenX - 1) * TilePixelWidth + _
                                    PixelOffsetX + TileBufferPixelOffsetX, (ScreenY - 1) * TilePixelHeight + _
                                    PixelOffsetY + TileBufferPixelOffsetY, 0, 1)
        ...
        For y = MinY To MaxY
            ...
            For X = MinX To MaxX
                ...
                If MapData(X, y).Graphic(2).GrhIndex <> 0 Then
                    Call DDrawTransGrhtoSurface(BackBufferSurface, MapData(X, y).Graphic(2), (ScreenX - 1) * _
                            TilePixelWidth + PixelOffsetX, (ScreenY - 1) * TilePixelHeight + PixelOffsetY, 1, 1)
                ...
                If bSelectSup Then
                    If CurLayer = 2 Then
                        If (X = SobreX) And (y = SobreY) Then
                            If frmConfigSup.MOSAICO.Value = vbChecked Then
                                Call DDrawTransGrhtoSurface(BackBufferSurface, CurrentGrh(((X + DespX) Mod mAncho) + _
                                        1, ((y + DespY) Mod MAlto) + 1), (ScreenX - 1) * TilePixelWidth + _
                                        PixelOffsetX, (ScreenY - 1) * TilePixelHeight + PixelOffsetY, 1, 1)
                            Else
                                Call DDrawTransGrhtoSurface(BackBufferSurface, CurrentGrh(0), (ScreenX - 1) * _
                                        TilePixelWidth + PixelOffsetX, (ScreenY - 1) * TilePixelHeight + _
                                        PixelOffsetY, 1, 1)
    ...
    Else
        ...
        If bSelectSup Then
            If CurLayer = 2 Then
                ...
                If frmConfigSup.MOSAICO.Value = vbChecked Then
                    Call DDrawTransGrhtoSurface(BackBufferSurface, CurrentGrh(((X + DespX) Mod mAncho) + 1, ((y + _
                            DespY) Mod MAlto) + 1), (ScreenX - 1) * TilePixelWidth + PixelOffsetX, (ScreenY - 1) * _
                            TilePixelHeight + PixelOffsetY, 1, 1)
                Else
                    Call DDrawTransGrhtoSurface(BackBufferSurface, CurrentGrh(0), (ScreenX - 1) * TilePixelWidth + _
                            PixelOffsetX, (ScreenY - 1) * TilePixelHeight + PixelOffsetY, 1, 1)
        ...
        For X = MinX To MaxX
            ...
            With MapData(X, y)
                ...
                If (.ObjGrh.GrhIndex <> 0) And bVerObjetos Then
                    Call DDrawTransGrhtoSurface(BackBufferSurface, .ObjGrh, PixelOffsetXTemp, PixelOffsetYTemp, 1, 1)
                ...
                If (.Graphic(3).GrhIndex <> 0) And bVerCapa(3) Then
                    'Draw
                    Call DDrawTransGrhtoSurface(BackBufferSurface, .Graphic(3), PixelOffsetXTemp, PixelOffsetYTemp, _
                            1, 1)
            ...
            For X = MinX To MaxX
                ...
                With MapData(X, y)
                    ...
                    If .Graphic(4).GrhIndex <> 0 Then
                        'Draw
                        Call DDrawTransGrhtoSurface(BackBufferSurface, .Graphic(4), (ScreenX - 1) * TilePixelWidth + _
                                PixelOffsetX, (ScreenY - 1) * TilePixelHeight + PixelOffsetY, 1, 1)
        ...
        For X = screenminX To screenmaxX
            ...
            With MapData(X, y)
                ...
                If (.TileExit.Map <> 0) And bTranslados Then
                    ...
                    Call DDrawTransGrhtoSurface(BackBufferSurface, Grh, PixelOffsetXTemp, PixelOffsetYTemp, 1, 0)
                ...
                If bTriggers Then
                    If .Trigger > 0 Then
                        Call TextDrawer.AddText(PixelOffsetXTemp + TilePixelWidth \ 2, PixelOffsetYTemp + _
                                TilePixelHeight \ 2, vbRed, CStr(.Trigger), True)
                ...
                If .Select Then
                    ...
                    BackBufferSurface.DrawBox PixelOffsetXTemp, PixelOffsetYTemp, PixelOffsetXTemp + TilePixelWidth, _
                            PixelOffsetYTemp + TilePixelHeight
...
Private Sub CharRender(ByVal CharIndex As Long, _
    ...
    With CharList(CharIndex)
        ...
        'Draw Body
        If .Body.Walk(.Heading).GrhIndex Then Call DDrawTransGrhtoSurface(BackBufferSurface, .Body.Walk(.Heading), _
                PixelOffsetX, PixelOffsetY, 1, 1)
    
        'Draw Head
        If .Head.Head(.Heading).GrhIndex Then Call DDrawTransGrhtoSurface(BackBufferSurface, .Head.Head(.Heading), _
                PixelOffsetX + .Body.HeadOffset.X, PixelOffsetY + .Body.HeadOffset.y, 1, 0)
...
Public Function InitTileEngine(ByVal setDisplayFormhWnd As Long, _
    ...
    Call SurfaceDB.Initialize(DirectDraw, False, DirGraficos, 90)
