    ...
    If Not MapData_Deshacer_Info(1).Libre Then
        ...
        For f = XMinMapSize To XMaxMapSize
            For j = YMinMapSize To YMaxMapSize
                ...
                If (MapData(f, j).NPCIndex <> 0 And MapData(f, j).NPCIndex <> MapData_Deshacer(1, f, j).NPCIndex) Or ( _
                        MapData(f, j).NPCIndex <> 0 And MapData_Deshacer(1, f, j).NPCIndex = 0) Then
...
Public Sub Superficie_Azar()
    ...
    If Cuantos > 0 Then
        ...
        For k = 1 To Cuantos
            ...
            Call InsertarGrh(tx, tY, frmConfigSup.MOSAICO.Value = vbChecked, bAutoCompletarSuperficies, _
...
Public Sub Superficie_Bordes()
    ...
    For X = XMinMapSize To XMaxMapSize
        For y = YMinMapSize To YMaxMapSize
            ...
            If X < MinXBorder Or X > MaxXBorder Or y < MinYBorder Or y > MaxYBorder Then
                Call InsertarGrh(X, y, frmConfigSup.MOSAICO.Value = vbChecked, False, frmMain.cInsertarBloqueo.Value, _
                        False)
    ...
    For X = XMinMapSize To XMaxMapSize
        For y = YMinMapSize To YMaxMapSize
            ...
            If MapData(X, y).NPCIndex > 0 Then
                If (Hostiles And NpcData(MapData(X, y).NPCIndex).Hostile) Or ((Hostiles = False) And (NpcData(MapData( _
                        X, y).NPCIndex).Hostile = False)) Then
...
Sub ClickEdit(ByVal Button As Integer, ByVal tx As Integer, ByVal tY As Integer)
    ...
    ElseIf Button = vbLeftButton Then
        ...
        ElseIf bSelectSup Then
            Call InsertarGrh(tx, tY, frmConfigSup.MOSAICO.Value = vbChecked, bAutoCompletarSuperficies, MapData(tx, _
                    tY).Blocked)
            '************** Place blocked tile
        ...
        ElseIf frmMain.cInsertarTrans.Value Then
            ...
            If frmMain.cInsertarTransOBJ.Value Then Call InsertarObjTranslado(tx, tY)
...
Public Sub GetMapData(ByVal X As Byte, ByVal y As Byte)
    ...
    With MapData(X, y)
        ...
        If .TileExit.Map > 0 Then
            ...
            frmMain.StatTxt.Text = frmMain.StatTxt.Text & " (Trans.: " & .TileExit.Map & "," & .TileExit.X & "," & _
                    .TileExit.y & ")"
        ...
        If .NPCIndex > 0 Then
            If NpcData(.NPCIndex).Hostile Then
                frmMain.StatTxt.Text = frmMain.StatTxt.Text & " (NPC-Hostil: " & .NPCIndex & " - " & NpcData( _
                        .NPCIndex).name & ")"
            Else
                frmMain.StatTxt.Text = frmMain.StatTxt.Text & " (NPC: " & .NPCIndex & " - " & NpcData(.NPCIndex).name _
                        & ")"
        ...
        If .OBJInfo.objindex > 0 Then
            frmMain.StatTxt.Text = frmMain.StatTxt.Text & " (Obj: " & .OBJInfo.objindex & " - " & ObjData( _
                    .OBJInfo.objindex).name & " - Cant.:" & .OBJInfo.Amount & ")"

        End If
    
        ' Capas
        frmMain.StatTxt.Text = frmMain.StatTxt.Text & ENDL & "Capa1: " & .Graphic(1).GrhIndex & " - Capa2: " & _
                .Graphic(2).GrhIndex & " - Capa3: " & .Graphic(3).GrhIndex & " - Capa4: " & .Graphic(4).GrhIndex
    ...
    If MaxSelectX Then 'Si tenemos un max, tenemos el otro, y también tenemos los min
        ...
        For y = MinSelectY To MaxSelectY
            For X = MinSelectX To MaxSelectX
                ...
                ElseIf bSelectSup Then
                    Call InsertarGrh(X, y, frmConfigSup.MOSAICO.Value = vbChecked, bAutoCompletarSuperficies, MapData( _
                            X, y).Blocked, False)
                ElseIf frmMain.cInsertarBloqueo.Value Then
                ...
                ElseIf frmMain.cInsertarTrans.Value Then
                    ...
                    If frmMain.cInsertarTransOBJ.Value Then Call InsertarObjTranslado(X, y, False)
...
Public Sub QuitarCapasMedias(ByVal X As Byte, _
                             ByVal y As Byte, _
                             Optional ByVal ConDeshacer As Boolean = True)
    Dim i As Byte
    
    If ConDeshacer Then Call modEdicion.Deshacer_Add("Quitar capas medias")
...
Public Function QuitarEstaCapa(ByVal X As Byte, _
                               ByVal y As Byte, _
                               Optional ByVal ConDeshacer As Boolean = True) As Boolean

    If MapData(X, y).Graphic(CurLayer).GrhIndex <> 0 Then
        If ConDeshacer Then Call modEdicion.Deshacer_Add("Quitar capa " & CurLayer)
...
Public Sub InsertarGrh(ByVal X As Byte, _
                       ByVal y As Byte, _
                       ByVal MOSAICO As Boolean, _
                       ByVal AutoCompletar As Boolean, _
    ...
    If MOSAICO And AutoCompletar Then
        If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar superficie autocompletada. Capa " & CurLayer)
        
        For OffsetX = 0 To mAncho - 1
            For OffsetY = 0 To MAlto - 1
                GrhIndex = CurrentGrh(((X + OffsetX + DespX) Mod mAncho) + 1, ((y + OffsetY + DespY) Mod MAlto) + _
                        1).GrhIndex
    ...
    Else
        ...
        With MapData(X, y)
            ...
            If .Graphic(CurLayer).GrhIndex <> GrhIndex Then
                If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar superficie. Capa " & CurLayer)
...
Public Sub InsertarBloq(ByVal X As Byte, _
                        ByVal y As Byte, _
                        Optional ByVal ConDeshacer As Boolean = True)

    If MapData(X, y).Blocked <> 1 Then
        If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar bloqueo")
...
Public Sub QuitarBloq(ByVal X As Byte, _
                      ByVal y As Byte, _
                      Optional ByVal ConDeshacer As Boolean = True)

    If MapData(X, y).Blocked <> 0 Then
        If ConDeshacer Then Call modEdicion.Deshacer_Add("Quitar bloqueo")
...
Public Sub InsertarObjTranslado(ByVal X As Byte, _
                                ByVal y As Byte, _
                                Optional ByVal ConDeshacer As Boolean = True)
    ...
    With MapData(X, y)
        ...
        If (Cfg_TrOBJ > 0) And (Cfg_TrOBJ <= NumOBJs) Then
            If ObjData(Cfg_TrOBJ).ObjType = 19 Then
                If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar Objeto de Translado")
...
Public Sub InsertarUnionManual(ByVal X As Byte, _
                               ByVal y As Byte, _
    ...
    With MapData(X, y).TileExit
        ...
        If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar Translado de Union Manual")
...
Public Sub InsertarTileExit(ByVal X As Byte, _
                            ByVal y As Byte, _
    ...
    With MapData(X, y).TileExit
        ...
        If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar Translado")
...
Public Sub QuitarTileExit(ByVal X As Byte, _
                          ByVal y As Byte, _
                          Optional ByVal ConDeshacer As Boolean)

    With MapData(X, y).TileExit

        If ConDeshacer Then Call modEdicion.Deshacer_Add("Quitar Translado")
...
Public Sub InsertarNpc(ByVal X As Byte, _
                       ByVal y As Byte, _
    ...
    With MapData(X, y)
        ...
        If NPCIndex <> .NPCIndex Then
            If .NPCIndex > 0 Then Call EraseChar(.CharIndex)
        
            If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar NPC " & IIf(NpcData(NPCIndex).Hostile, _
                    "Hostil", "No Hostil"))
...
Public Sub QuitarNpc(ByVal X As Byte, _
                     ByVal y As Byte, _
                     Optional ByVal ConDeshacer As Boolean = True)
    ...
    With MapData(X, y)
        ...
        If .NPCIndex > 0 Then
            If ConDeshacer Then Call modEdicion.Deshacer_Add("Quitar NPC")
...
Public Sub InsertarObjeto(ByVal X As Byte, _
                          ByVal y As Byte, _
    ...
    With MapData(X, y)
        ...
        If objindex > 0 Then
            If .OBJInfo.objindex <> objindex Or .OBJInfo.Amount <> Amount Then
                If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar Objeto")
...
Public Sub QuitarObjeto(ByVal X As Byte, _
                        ByVal y As Byte, _
                        Optional ByVal ConDeshacer As Boolean)
    ...
    With MapData(X, y)
        ...
        If .OBJInfo.objindex <> 0 Then
            If ConDeshacer Then Call modEdicion.Deshacer_Add("Quitar objeto")
...
Public Sub InsertarTrigger(ByVal X As Byte, _
                           ByVal y As Byte, _
                           ByVal Trigger As Byte, _
                           Optional ByVal ConDeshacer As Boolean)

    With MapData(X, y)

        If .Trigger <> Trigger Then
            If ConDeshacer Then Call modEdicion.Deshacer_Add("Insertar Trigger " & Trigger)
