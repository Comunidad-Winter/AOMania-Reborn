...
Attribute ButtonStyle.VB_Description = "Various operating system button styles"
    ...
    If myProps.bShape < lv_Round3D Or myProps.bShape < lv_CustomFlat And (myProps.bBackStyle = lv_hover Or lastStyle _
            = lv_hover) Then CreateButtonRegion        ' re-create the button shape
...
Attribute CaptionAlign.VB_ProcData.VB_Invoke_PropertyPut = ";Appearance"
    ...
    If myImage.Align > lv_RightOfCaption And nAlign < vbCenter And (myImage.SourceSize.X + myImage.SourceSize.y) > 0 _
            Then
        ...
        If UserControl.Ambient.UserMode = False Then
            ...
            MsgBox "When button images are aligned top/bottom center, " & vbCrLf & _
                    "button captions can only be center aligned", vbOKOnly + vbInformation
...
Attribute ButtonShape.VB_ProcData.VB_Invoke_PropertyPut = ";Appearance"
    ...
    If nShape > lv_RoundFlat Then   ' custom shapes
        If Me.Picture Is Nothing Or myImage.type = CI_ICON Then
            If Not Ambient.UserMode Then MsgBox _
                    "The Picture Property must be assigned first and must be a bitmap or JPEG.", vbInformation + _
                    vbOKOnly
            Exit Property
        Else
    ...
    If myProps.bShape > lv_RoundFlat Then   ' custom shapes
        ...
        Else
            ...
            If myImage.type = CI_ICON Then
                ...
                If Not Ambient.UserMode Then MsgBox _
                        "Icons cannot be used for custom buttons. Only use bitmaps or JPEGs." & vbCrLf & _
                        "Button was changed to Rectangular shaped.", vbInformation + vbOKOnly
...
Attribute PictureSize.VB_Description = "Various sizes for images used on buttons. Last 2 options center image automatically."
    ...
    If myProps.bShape > lv_RoundFlat Then
        If Not Ambient.UserMode Then MsgBox "The picture size cannot be changed for Shaped buttons", vbInformation + _
                vbOKOnly
        Exit Property
...
Attribute GradientColor.VB_Description = "Secondary color used for gradient shades. The BackColor property is the primary color."
    ...
    If (myProps.bLockHover = lv_LockTextandBackColor Or myProps.bLockHover = lv_LockBackColorOnly) And _
                ...
                Case lv_Custom3DBorder, lv_CustomFlat
                    ...
                    Else
                        ...
                        If ((myProps.bStatus And 1) = 1) Then       ' got the focus
                            If myProps.bShape = lv_Custom3DBorder Then tRgn = ButtonDC.ClipRgn Else tRgn = _
                                    ButtonDC.ClipBorder
                            If myProps.bValue And myProps.bBackStyle <> 2 Then FocusColor = ShadeColor( _
                                    adjBackColorDn, -&H20, False)
...
Private Sub CalculateBoundingRects(bNormalizeImage As Boolean)
    ...
    If Len(myProps.bCaption) Then
        ...
        cRect.Bottom = ScaleHeight - 8 - (myImage.Size * Abs(CInt(bImgHeightAdj = True And myImage.Align > _
                lv_RightOfCaption)))
        ...
        Select Case myProps.bCaptionAlign
            ...
            Case vbCenter
                ...
                If imgOffset.Left > 0 And myImage.Align = lv_LeftOfCaption Then
                    OffsetRect tRect, (adjWidth - (imgOffset.Left + cRect.Right + 4)) \ 2 + lEdge + 4 + _
                            imgOffset.Left, 0
                Else
                    ...
                    Else
                        OffsetRect tRect, ((adjWidth - (imgOffset.Left + imgOffset.Right)) - cRect.Right) \ 2 + lEdge _
                                + imgOffset.Left, 0
...
Private Sub NormalizeImage(newSizeX As Long, newSizeY As Long, rtnRgn As Long)
    ...
    For y = 0 To newSizeY
        ...
        For X = 0 To newSizeX
            ...
            If tRed >= valRed - 3 And tRed <= valRed + 3 And tBlue >= valBlue - 3 And tBlue <= valBlue + 3 And tGreen _
                    >= valGreen - 3 And tGreen <= valGreen + 3 Then
...
Private Sub DrawButtonBackground(bColor As Long, _
    ...
    If bTimerActive And (((myProps.bMode = lv_CommandButton And isDown = 0) Or (myProps.bValue = False And _
            myProps.bMode > lv_CommandButton))) Then
    ...
    If myProps.bGradient And myProps.bValue = False And myProps.bShape < lv_CustomFlat Then
        If bTimerActive = True And ((myProps.bStatus And 6) = 6) = False And (myProps.bGradientColor <> _
                myProps.bBackHover) Then
...
Private Sub DrawCustomBorders(ActiveRegion As Integer)
    ...
    If bTimerActive = True Or ((myProps.bStatus And 6) = 6) Or myProps.bValue = True Or myProps.bBackStyle <> 5 Then
        ...
        If myProps.bShape < lv_RoundFlat Or myProps.bShape > lv_Round3D Then
            ...
            If myProps.bBackStyle = 5 Then
                If myProps.bValue = True Or (myProps.bBackStyle = 5 And (myProps.bShape <> lv_CustomFlat And _
                        myProps.bShape <> lv_RoundFlat)) Then
                    myProps.bGradient = lv_Top2Bottom
                    DrawGradient vbWhite, vbGray
            ...
            If (((myProps.bStatus And 6) = 6) Or myProps.bValue = True) And (myProps.bShape <> lv_CustomFlat And _
                    myProps.bShape <> lv_RoundFlat) Then
                SelectClipRgn ButtonDC.hdc, ButtonDC.ClipBorder
                myProps.bGradient = lv_Top2Bottom
...
Private Sub DrawButtonBorder(polyPts() As POINTAPI, _
    ...
    For i = 5 To 9
        polyPts(i).X = polyPts(i - 5).X + Choose(i - 4, polyOffset.X - 1, -1 - polyOffset.X, 1 - polyOffset.y, 1 + _
                polyOffset.y, -1, -1)
    ...
    For i = 0 To 13
        ...
        If (i <> 4 And i <> 9) Then
            ...
            If polyColors(xColorRef) > -1 Then
                ...
                If lastcolor <> polyColors(xColorRef) Then SetButtonColors True, ButtonDC.hdc, cObj_Pen, polyColors( _
                        xColorRef), , , , lBorderStyle
...
Private Sub DrawFocusRectangle(fColor As Long, _
    ...
    Else
        ...
        If bOnText = True Then
            ...
            If myProps.bRect.Left > 4 + myProps.bSegPts.X And myProps.bRect.Right < myProps.bSegPts.y - 4 Then _
                    focusOffset = 2 Else focusOffset = 1
...
Private Sub DrawCaptionIcon(bColor As Long, _
    ...
    Else
        ...
        If (myProps.bCaptionStyle And UserControl.Enabled = True) Then
            ...
            DrawText ButtonDC.hdc, sCaption, Len(sCaption), tRect, DT_WORDBREAK Or Choose(myProps.bCaptionAlign + 1, _
    ...
    DrawText ButtonDC.hdc, sCaption, Len(sCaption), tRect, DT_WORDBREAK Or Choose(myProps.bCaptionAlign + 1, DT_LEFT, _
            DT_RIGHT, DT_CENTER)
...
Private Sub DrawButton_Win95(polyPts() As POINTAPI, _
    ...
    If (myProps.bShape = lv_RoundFlat And (bTimerActive = True Or ((myProps.bStatus And 6) = 6))) Or myProps.bShape > _
            lv_RoundFlat Then
        polyColors(12) = 0
    Else
        lastClipRgn = 1
...
Private Sub DrawButton_WinXP(polyPts() As POINTAPI, _
    ...
    ElseIf myProps.bShape < lv_RoundFlat Then
        ...
        If UserControl.Enabled Then
            ...
            liteShade = lColor
            ' inner Rectangle
            polyColors(1) = Choose(ActiveStatus + 1, ShadeColor(lColor, -&HA, True), &HF0D1B5, ShadeColor(liteShade, _
                    -&H16, True), &H6BCBFF)
            polyColors(2) = Choose(ActiveStatus + 1, ShadeColor(lColor, &HA, True), &HF7D7BD, ShadeColor(liteShade, _
                    -&H18, True), &H8CDBFF)
            polyColors(3) = Choose(ActiveStatus + 1, ShadeColor(lColor, -&H18, True), &HF0D1B5, lColor, &H6BCBFF)
            polyColors(4) = Choose(ActiveStatus + 1, ShadeColor(lColor, -&H20, True), &HE7AE8C, ShadeColor(liteShade, _
                    &HA, True), &H31B2FF)
            ' middle Rectangle
            polyColors(5) = Choose(ActiveStatus + 1, ShadeColor(lColor, -&H5, True), &HE7AE8C, ShadeColor(liteShade, _
                    -&H20, True), &H31B2FF)
            polyColors(6) = Choose(ActiveStatus + 1, ShadeColor(lColor, &H10, True), &HFFDFBF, ShadeColor(liteShade, _
                    -&H20, True), &HA6E9FF)
            polyColors(7) = Choose(ActiveStatus + 1, ShadeColor(lColor, -&H24, True), &HE7AE8C, ShadeColor(liteShade, _
                    &H5, True), &H31B2FF)
            polyColors(8) = Choose(ActiveStatus + 1, ShadeColor(lColor, -&H30, True), &HEF826B, ShadeColor(liteShade, _
                    &H10, True), &H96E7&)
            lColor = &H733C00
        Else
...
Private Sub DrawButton_Hover(polyPts() As POINTAPI, _
    ...
    If ((myProps.bShape = lv_RoundFlat And (bTimerActive = True Or ((myProps.bStatus And 6) = 6))) Or myProps.bShape _
            > lv_RoundFlat) Or (myProps.bShape = lv_CustomFlat And myProps.bValue = False) Then
        polyColors(12) = 0
    Else
        lastClipRgn = 1
