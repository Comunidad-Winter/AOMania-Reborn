...
Private Declare Function GdipCloneBitmapArea _
                Lib "gdiplus" (ByVal X As Single, _
                               ...
                               ByRef dstBitmap As Long) As Status
...
Private Declare Function GdipGetImagePixelFormat _
...
Private Declare Function GdipBitmapConvertFormat _
                Lib "gdiplus" (ByVal pInputBitmap As Long, _
...
Private Declare Function GdipInitializePalette _
                Lib "gdiplus" (ByVal palette As Any, _
                               ByVal PaletteType As PaletteType, _
...
Public Function Execute(ByVal lReturn As Status) As Status
    ...
    Else
        lCurErr = lReturn
        MsgBox GdiErrorString(lReturn) & " GDI+ Error:" & lReturn, vbOKOnly, "GDI Error"
...
Private Function ConvertTo1bppIndexedAndSaveAsTiffGDI5(ByVal sFileName As String, _
                                                       ByVal lInBitmap As Long, _
                                                       Optional ByVal eTifCompression As EncoderValueConstants = EncoderValueCompressionNone) As Status
    ...
    If GetEncoderClsid(mimeTIFF, tPicEncoder) = True Then
        ...
        If Execute(GdipGetImageDimension(lInBitmap, sWidth, sHeight)) = OK Then
            ...
            If Execute(GdipCloneBitmapArea(0, 0, sWidth, sHeight, PixelFormat1bppIndexed, lInBitmap, lNewBitmap)) = _
                    OK Then
            
                ' Speichert lNewBitmap als
                ' 1bppIndexed Tiff
                ConvertTo1bppIndexedAndSaveAsTiffGDI5 = Execute(GdipSaveImageToFile(lNewBitmap, StrPtr(sFileName), _
...
Private Function ConvertTo1bppIndexedAndSaveAsTiffGDI6(ByVal sFileName As String, _
                                                       ByVal lInBitmap As Long, _
                                                       Optional ByVal eTifCompression As EncoderValueConstants = EncoderValueCompressionNone) As Status
    ...
    If GetEncoderClsid(mimeTIFF, tPicEncoder) = True Then
        ...
        If Execute(GdipGetImageDimension(lInBitmap, sWidth, sHeight)) = OK Then
            ...
            If Execute(GdipGetImagePixelFormat(lInBitmap, ePixelFormat)) = OK Then
                ...
                ' -> lNewBitmap
                If Execute(GdipCloneBitmapArea(0, 0, sWidth, sHeight, ePixelFormat, lInBitmap, lNewBitmap)) = OK Then
              
                    ' optimierte 1bppIndexed Palette
                    ' für lNewBitmap erzeugen
                    If Execute(GdipInitializePalette(VarPtr(tCPal(0)), PaletteTypeOptimal, tCPal(0).Count, CLng(Abs( _
                            False)), lNewBitmap)) = OK Then
                      
                        ' lNewBitmap zu 1bppIndexed Bitmap mit
                        ' erzeugter Palette konvertieren
                        If Execute(GdipBitmapConvertFormat(lNewBitmap, PixelFormat1bppIndexed, _
                                DitherTypeDualSpiral8x8, PaletteTypeOptimal, VarPtr(tCPal(0)), 0)) = OK Then
                        
                            ' Speichert lNewBitmap als
                            ' 1bppIndexed Tiff mit
                            ' optimierter Palette
                            ConvertTo1bppIndexedAndSaveAsTiffGDI6 = Execute(GdipSaveImageToFile(lNewBitmap, StrPtr( _
                                    sFileName), tPicEncoder, tEncoderParameters))
...
Private Function SaveAsTiff(ByVal sFileName As String, _
    ...
    If GetEncoderClsid(mimeTIFF, tPicEncoder) = True Then
        ...
        SaveAsTiff = Execute(GdipSaveImageToFile(lInBitmap, StrPtr(sFileName), tPicEncoder, tEncoderParameters))
...
Public Function SavePictureAsTiff(ByVal Pic As StdPicture, _
                                  ...
                                  Optional ByVal eTifCompression As EncoderValueConstants = EncoderValueCompressionNone) As Boolean
    ...
    If Execute(GdipCreateBitmapFromHBITMAP(Pic.Handle, 0, lBitmap)) = OK Then
    
        ' Kompressionstyp
        Select Case eTifCompression
      
            Case EncoderValueCompressionNone, EncoderValueCompressionLZW
          
                lRet = SaveAsTiff(sFileName, lBitmap, eTifCompression)
            ...
            Case Else 'RLE, CCITT3, CCITT4
                ...
                If UseGDI6 Then
                    ...
                    lRet = ConvertTo1bppIndexedAndSaveAsTiffGDI6(sFileName, lBitmap, eTifCompression)
                Else
                    ' oder GDI+ v5.xx
                
                    ' !!! ab GDI+ Version 5.xx und höher !!!
                    lRet = ConvertTo1bppIndexedAndSaveAsTiffGDI5(sFileName, lBitmap, eTifCompression)
...
Public Function SavePictureAsPNG(ByVal Pic As StdPicture, _
    ...
    If Execute(GdipCreateBitmapFromHBITMAP(Pic.Handle, 0, lBitmap)) = OK Then
        ...
        If GetEncoderClsid(mimePNG, tPicEncoder) = True Then
            ...
            If Execute(GdipSaveImageToFile(lBitmap, StrPtr(sFileName), tPicEncoder, ByVal 0)) = OK Then
