    ...
    For y = YMinMapSize To YMaxMapSize
        For X = XMinMapSize To XMaxMapSize
            ...
            If MapData(X, y).OBJInfo.objindex > 0 Then
                txtInfo.Text = txtInfo.Text & vbCrLf & X & "," & y & " tiene " & MapData(X, y).OBJInfo.Amount & _
                        " del Objeto " & MapData(X, y).OBJInfo.objindex & " - " & ObjData(MapData(X, _
                        y).OBJInfo.objindex).name
        ...
        For X = XMinMapSize To XMaxMapSize
            ...
            If NPCIndex > 0 Then
                If NpcData(NPCIndex).Hostile Then
                    txtInfo.Text = txtInfo.Text & vbCrLf & X & "," & y & " tiene " & NpcData(NPCIndex).name & _
                            " (Hostil)"
                Else
    ...
    For y = YMinMapSize To YMaxMapSize
        For X = XMinMapSize To XMaxMapSize
            ...
            If MapData(X, y).TileExit.Map > 0 Then
                txtInfo.Text = txtInfo.Text & vbCrLf & X & "," & y & " nos traslada a la posición " & MapData(X, _
                        y).TileExit.X & "," & MapData(X, y).TileExit.y & " del Mapa " & MapData(X, y).TileExit.Map

                If ((X < 20 And MapData(X, y).TileExit.X < 20) Or (X > 80 And MapData(X, y).TileExit.X > 80)) And (X _
                        <> MapData(X, y).TileExit.X) Then
                    txtInfo.Text = txtInfo.Text & " (X sospechoso)"

                End If

                If ((y < 20 And MapData(X, y).TileExit.y < 20) Or (y > 80 And MapData(X, y).TileExit.y > 80)) And (y _
                        <> MapData(X, y).TileExit.y) Then
