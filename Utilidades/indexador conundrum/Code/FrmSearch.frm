...
Public Sub HacerBusquedaR()
    ...
    For i = 1 To grhCount
        ...
        If GrhData(i).NumFrames > 0 Then
            ...
            For ii = i + 1 To grhCount

                If Bfin Then Exit Sub
                If GrhData(i).FileNum = GrhData(ii).FileNum And GrhData(i).NumFrames = GrhData( _
                        ii).NumFrames And GrhData(i).sX = GrhData(ii).sX And GrhData(i).sY = GrhData(ii).sY _
                        And GrhData(i).Frames(1) = GrhData(ii).Frames(1) And GrhData(i).Frames(2) = GrhData( _
                        ii).Frames(2) And GrhData(i).Frames(2) = GrhData(ii).Frames(2) Then
    ...
    For ii = 1 To UltimoGrafiCoRevisar
        ...
        If ExisteBMPActual = ResourceFile Or (ResourceFile = 3 And ExisteBMPActual > 0) Then
            ...
            If Not indexado Then
                ...
                ListRepetidos.AddItem ii & ".bmp NO esta indexado. El grafico esta en : " & StringRecurso( _
                        ExisteBMPActual) & stringGrap
...
Private Sub IAnimacion_Click()
    ...
    If IsNumeric(bmpstring) Then
        ...
        If BMPBuscado > 0 And BMPBuscado <= grhCount Then
