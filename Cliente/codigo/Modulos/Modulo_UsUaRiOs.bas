        ...
        Do While .Stats.Exp >= .Stats.ELU
            ...
            Else
                ...
                If .Clase = "TRABAJADOR" Then
                    Pts = Pts + 10
                ElseIf .Clase = "Gladiador Magico" Then
                    Pts = Pts + 7
                Else
                    Pts = Pts + 5
    ...
    With UserList(UserIndex)
        ...
        If .PartyIndex <> 0 Then
            ...
            If .Stats.MinHP <> 0 And .Stats.MaxHP <> 0 Then
                ...
                Call SendData(SendTarget.ToPartyArea, UserIndex, .pos.Map, "POY" & CInt((.Stats.MinHP / .Stats.MaxHP) * 100))
...
Sub SendUserInvTxt(ByVal sendIndex As Integer, ByVal UserIndex As Integer)
    ...
    For j = 1 To MAX_INVENTORY_SLOTS
        ...
        If UserList(UserIndex).Invent.Object(j).ObjIndex > 0 Then
            Call SendData(SendTarget.toIndex, sendIndex, 0, "|| Objeto " & j & " " & ObjData(UserList(UserIndex).Invent.Object(j).ObjIndex).Name & _
    ...
    If UserList(UserIndex).flags.Invisible = 1 Or UserList(UserIndex).flags.Oculto = 1 Then
        ...
        If UserList(UserIndex).PartyIndex <> 0 Then
            ...
            If UserList(UserIndex).Stats.MinHP <> 0 And UserList(UserIndex).Stats.MaxHP <> 0 Then
                ...
                Call SendData(SendTarget.ToPartyArea, UserIndex, UserList(UserIndex).pos.Map, "POY" & CInt((UserList(UserIndex).Stats.MinHP / _
                    UserList(UserIndex).Stats.MaxHP) * 100))
...
Sub WarpUserChar(ByVal UserIndex As Integer, ByVal Map As Integer, ByVal X As Integer, ByVal Y As Integer, Optional ByVal FX As Boolean = False)
    ...
    With UserList(UserIndex)
        ...
        If FX And .flags.AdminInvisible = 0 Then 'FX
            Call SendData(SendTarget.ToPCArea, UserIndex, .pos.Map, "TW" & SND_WARP)
    ...
    Select Case Clase
        ...
        Case "GLADIADOR MAGICO"
            ...
            Select Case UserList(UserIndex).Stats.UserAtributos(eAtributos.Constitucion)
                ...
                Case 21
                    AumentoHP = RandomNumber(GLCONST21MINVIDA, GLCONST21MAXVIDA)

                Case 20
                    AumentoHP = RandomNumber(GLCONST20MINVIDA, GLCONST20MAXVIDA)

                Case 19
                    AumentoHP = RandomNumber(GLCONST19MINVIDA, GLCONST19MAXVIDA)

                Case 18
                    AumentoHP = RandomNumber(GLCONST18MINVIDA, GLCONST18MAXVIDA)
               
                Case 17
                    AumentoHP = RandomNumber(GLCONST17MINVIDA, GLCONST17MAXVIDA)
                
                Case Else
                    AumentoHP = RandomNumber(GLCONSTOTROMINVIDA, GLCONSTOTROMAXVIDA)

            End Select
            
            AumentoHIT = 3
            AumentoMANA = UserList(UserIndex).Stats.UserAtributos(eAtributos.Inteligencia) * 1.7
            AumentoSTA = AumentoSTDef
...
Public Sub SetearInv(ByVal UserIndex As Integer, ByVal Clase As String, ByVal Raza As String)
    ...
    With UserList(UserIndex).Invent
        ...
        Select Case Clase
            ...
            Case "CAZADOR", "ARQUERO", "GLADIADOR MAGICO"
                Slot = Slot + 1
