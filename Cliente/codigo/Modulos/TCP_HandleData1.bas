    ...
    Select Case UCase$(Left$(rData, 2))
        ...
        Case "OH" 'Tirar item
            ...
            If Not UserList(UserIndex).flags.SeguroObjetos Then
                ...
                Call SendData(SendTarget.toIndex, UserIndex, 0, "||Seguro de objetos activado, para desativar apretar la tecla ''X''." & _
                    FONTTYPE_INFO)
                Exit Sub
        ...
        Case "UK"
            ...
            Select Case val(rData)
                ...
                Case Ocultarse
                    ...
                    If MapInfo(UserList(UserIndex).pos.Map).OcultarSinEfecto = 1 Then

                        '[CDT 17-02-2004]
                        If Not UserList(UserIndex).flags.UltimoMensaje = 54 Then
                            Call SendData(SendTarget.toIndex, UserIndex, 0, "||No podes ocultarte si estas en este mapa." & FONTTYPE_INFO)
                            UserList(UserIndex).flags.UltimoMensaje = 54
        ...
        Case "WLC" 'Click izquierdo en modo trabajo
            ...
            Select Case tLong
                ...
                Case Magia
                    ...
                    Else
                        Call SendData(SendTarget.toIndex, UserIndex, 0, "||¡Primero selecciona el hechizo que queres lanzar y después lanzá!" & _
                            FONTTYPE_INFO)
                ...
                Case Pesca
                    ...
                    If ((LegalPos(UserList(UserIndex).pos.Map, UserList(UserIndex).pos.X - 1, UserList(UserIndex).pos.Y, True) Or LegalPos(UserList( _
                        UserIndex).pos.Map, UserList(UserIndex).pos.X, UserList(UserIndex).pos.Y - 1, True) Or LegalPos(UserList( _
                        UserIndex).pos.Map, UserList(UserIndex).pos.X + 1, UserList(UserIndex).pos.Y, True) Or LegalPos(UserList( _
                        UserIndex).pos.Map, UserList(UserIndex).pos.X, UserList(UserIndex).pos.Y + 1, True)) And UserList( _
                        UserIndex).flags.Navegando = 0) Or UserList(UserIndex).flags.Navegando = 1 Then
                        Call SendData(SendTarget.toIndex, UserIndex, 0, _
                            "||No puedes pescar navegando, debes acercarte a la orilla para poder pescar!." & FONTTYPE_INFO)
                        Exit Sub
                    ...
                    If CI > 0 Then
                        If Npclist(CI).flags.Domable > 0 Then
                            ...
                            If Npclist(CI).flags.AttackedBy <> "" Then
                                Call SendData(SendTarget.toIndex, UserIndex, 0, "||No podés domar una criatura que está luchando con un jugador." & _
                                    FONTTYPE_INFO)
                                Exit Sub
    ...
    Select Case UCase$(Left$(rData, 6))
        ...
        Case "DESPHE" 'Mover Hechizo de lugar
            ...
            If UserList(UserIndex).flags.SeguroHechizos Then
                Call SendData(SendTarget.toIndex, UserIndex, 0, "||Seguro de hechizos activado, para desactivar apreta la tecla ''*''.!!" & _
                    FONTTYPE_ORO)
                Exit Sub
