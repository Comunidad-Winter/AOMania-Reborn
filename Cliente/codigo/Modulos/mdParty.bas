...
Public Sub CrearParty(ByVal UserIndex As Integer)
    ...
    If UserList(UserIndex).PartyIndex = 0 Then
        If UserList(UserIndex).flags.Muerto = 0 Then
            ...
            Else
                Call SendData(SendTarget.toIndex, UserIndex, 0, "|| No tienes suficientes puntos de liderazgo para liderar una party." & _
                    FONTTYPE_PARTY)
...
Public Sub SolicitarIngresoAParty(ByVal UserIndex As Integer)
    ...
    Else
        Call SendData(SendTarget.toIndex, UserIndex, 0, "|| Para ingresar a una party debes hacer click sobre el fundador y luego escribir /PARTY" _
            & FONTTYPE_PARTY)
        UserList(UserIndex).PartySolicitud = 0
            ...
            If UserList(NewMember).PartyIndex = 0 Then
                If Not UserList(Leader).flags.Muerto = 1 Then
                    If Not UserList(NewMember).flags.Muerto = 1 Then
                        If UserList(NewMember).PartySolicitud = PI Then
                            If Parties(PI).PuedeEntrar(NewMember, razon) Then
                                If Parties(PI).NuevoMiembro(NewMember) Then
                                    Call Parties(PI).MandarMensajeAConsola(UserList(Leader).Name & " ha aceptado a " & UserList(NewMember).Name & _
                                        " en la party.", "Servidor")
                                    UserList(NewMember).PartyIndex = PI
                                    UserList(NewMember).PartySolicitud = 0
                                    
                                    Call EnviarHP(NewMember)
                                Else
                                    'no pudo entrar
                                    'ACA UNO PUEDE CODIFICAR OTRO TIPO DE ERRORES...
                                    Call SendData(SendTarget.ToAdmins, Leader, 0, "|| AOMania> CATASTROFE EN PARTIES, NUEVOMIEMBRO DIO FALSE! :S " _
                                        & FONTTYPE_PARTY)
                        ...
                        Else
                            Call SendData(SendTarget.toIndex, Leader, 0, "|| " & UserList(NewMember).Name & _
                                " no ha solicitado ingresar a tu party." & FONTTYPE_PARTY)
                            Exit Sub
...
Public Sub ActualizaExperiencias()
    ...
    If Not PARTY_EXPERIENCIAPORGOLPE Then
        ...
        'Call SendData(SendTarget.toall, 0, 0, "||AOMania> Distribuyendo experiencia en todas las parties." & _
         FONTTYPE_SERVER)
        ...
        'Call SendData(SendTarget.toall, 0, 0, "||AOMania> Experiencia distribuida existosamente." & FONTTYPE_SERVER)
        Call SendData(SendTarget.toall, 0, 0, "BKW")
        haciendoBK = False
