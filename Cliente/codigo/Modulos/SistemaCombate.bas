    ...
    With UserList(UserIndex)
        ...
        If .Invent.WeaponEqpObjIndex > 0 Then
            ...
            If NpcIndex > 0 Then
                ...
                If Arma.Proyectil = 1 Then
                    ModifClase = ModClase(Clase).AtaqueProyectiles
        ...
        If UserList(UserIndex).GranPoder = 1 And GranPoder.TipoAura = hGranPoder.Daño Then
            ...
            ElseIf .Invent.WeaponEqpObjIndex = ObjEspadaNormal Or .Invent.WeaponEqpObjIndex = ObjEspadaAse Or .Invent.WeaponEqpObjIndex = ObjArcoNormal Then
                If NpcIndex > 0 Then
                    CalcularDaño = (3 * DañoArma + ((DañoMaxArma / 5) * MaximoInt(0, .Stats.UserAtributos(eAtributos.Fuerza) - 15)) + DañoUsuario) * _
                        ModifClase + val((3 * DañoArma + ((DañoMaxArma / 5) * MaximoInt(0, .Stats.UserAtributos(eAtributos.Fuerza) - 15)) + DañoUsuario) * _
                        ModifClase / 15) * 2
                Else
                    CalcularDaño = (3 * DañoArma + ((DañoMaxArma / 5) * MaximoInt(0, .Stats.UserAtributos(eAtributos.Fuerza) - 15)) + DañoUsuario) * _
                        ModifClase * 2
...
Public Sub UserDañoNpc(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    If UserList(UserIndex).pos.Map = MapaCasaAbandonada1 Or UserList(UserIndex).pos.Map = MapaCasaAbandonada2 Then
    ...
    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).pos.Map, "||" & vbYellow & "°- " & Daño & "!°" & CStr(Npclist( _
    ...
    If UserList(AtacanteIndex).Invent.WeaponEqpObjIndex > 0 Then
        If RandomNumber(1, 100) <= ObjData(UserList(AtacanteIndex).Invent.WeaponEqpObjIndex).Pegadoble Then
            ...
            If TeCritico = 10 Then
                ...
                Call SendData(SendTarget.toIndex, VictimaIndex, 0, "N4" & Lugar & "," & Round(Daño * 1.1, 0) * 1.1 & "," & UserList( _
                    AtacanteIndex).Name)
...
Public Function PuedeAtacar(ByVal AttackerIndex As Integer, ByVal VictimIndex As Integer) As Boolean
    ...
    If MapInfo(UserList(VictimIndex).pos.Map).Pk = False Then
        ...
        If esTemplario(AttackerIndex) Then
            If UserList(VictimIndex).pos.Map <> MapaNoValidoTemplario Then
                Call SendData(SendTarget.toIndex, AttackerIndex, 0, "||¡Huye de la ciudad! Estás siendo atacado y no podrás defenderte." & _
                    FONTTYPE_WARNING)
                PuedeAtacar = True
                Exit Function
        ...
        If esNemesis(AttackerIndex) Then
            If UserList(VictimIndex).pos.Map <> MapaNoValidoNemesis Then
                Call SendData(SendTarget.toIndex, AttackerIndex, 0, "||¡Huye de la ciudad! Estás siendo atacado y no podrás defenderte." & _
                    FONTTYPE_WARNING)
                PuedeAtacar = True
                Exit Function
