        ...
        For X = Npclist(NpcIndex).pos.X - 10 To Npclist(NpcIndex).pos.X + 10   '5 tiles in every direction
            ...
            If X > MinXBorder And X < MaxXBorder And Y > MinYBorder And Y < MaxYBorder Then
                ...
                If MapData(Npclist(NpcIndex).pos.Map, X, Y).UserIndex > 0 Then
                    ...
                    If UserList(tmpUserIndex).flags.Muerto = 0 And UserList(tmpUserIndex).flags.Invisible = 0 And UserList( _
                        tmpUserIndex).flags.Oculto = 0 And UserList(tmpUserIndex).flags.Privilegios = PlayerType.User Then
