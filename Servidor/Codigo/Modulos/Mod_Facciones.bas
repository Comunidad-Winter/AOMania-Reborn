...
Public Sub IniciarEventoFaccion()
       ...
       Select Case i
             ...
             Case 1 'Banderbill (Armada del Clero)
                  EventoFaccion.Map = 59
                  NpcPos.Map = EventoFaccion.Map
                  NpcPos.X = 50
                  NpcPos.Y = 41
                  
             Case 2 'Caosbill (Armada del Abaddon)
             Case 3 'Arhal (Armada de los Caballeros de la Tiniebla)
             Case 4 'Ciudad templaria (Armada de la Orden Templaria)
             
       End Select
       
       Call SpawnNpc(NpcReyFaccion, NpcPos, True, False)
       Call ChangeNPCReyFaccion(EventoFaccion.Map)
       
       Call SendData(ToAll, 0, 0, "||La ciudad de bandebill esta siendo atacada!!" & FONTTYPE_GUILD)
       
End Sub

Public Sub ChangeNPCReyFaccion(ByVal Map As Integer)
     
     Dim X As Byte, Y As Byte, NI As Integer
        ...
        Case 59
          ...
          For X = 1 To 100
            For Y = 1 To 100
                ...
                NI = val(MapData(Map, X, Y).NpcIndex)
                
                If NI > 0 Then
                   
                   If Npclist(NI).Numero = NpcReyFaccion Then
                      
                      Npclist(NI).Name = "Rey del Clero"
                      Npclist(NI).char.Body = "218"
                      Call ChangeNPCChar(ToMap, 0, Map, NI, Npclist(NI).char.Body, Npclist(NI).char.Head, Npclist(NI).char.heading)
                      Call SendData(ToMap, 0, Map, "CNN" & Npclist(NI).char.CharIndex & "," & Npclist(NI).Name)
                   End If
