...
Sub DarCuerpoYCabeza(ByRef UserBody As Integer, ByRef UserHead As Integer, ByVal Raza As String, ByVal Gen As String, Clase As String)
    ...
    Select Case UCase$(Gen)
        ...
        Case "HOMBRE"
            ...
            If Clase = "Gladiador Magico" Then
                UserHead = 10
                UserBody = 1
            End If
...
Sub ConnectNewUser(UserIndex As Integer, _
    ...
    US17 As String, _
    US18 As String, _
    US19 As String, US20 As String, US21 As String, US22 As String, UserEmail As String, Hogar As String, ByVal HdSerial As String)
    ...
    If UserClase = "Gladiador Magico" Then
        If Not FileExist(CharPath & UCase$(UserList(UserIndex).GladiadorR) & ".chr", vbNormal) = True Then
            Call SendData(toIndex, UserIndex, 0, "ERRNo puedes crear un Gladiador Magico.")
            Exit Sub
        End If
        Call WriteVar(CharPath & UCase$(UserList(UserIndex).GladiadorR) & ".chr", "INIT", "Gladiador", "1")
        Call SendData(toIndex, UserIndex, 0, "DGLA")
    End If
    ...
    End Select
    If UserList(UserIndex).Clase = "Gladiador Magico" Then
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Fuerza) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Fuerza) + 3
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Inteligencia) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Inteligencia) + 3
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Agilidad) + 3
        UserList(UserIndex).Stats.UserAtributos(eAtributos.Constitucion) = UserList(UserIndex).Stats.UserAtributos(eAtributos.Constitucion) + 3
    ...
    Call DarCuerpoYCabeza(UserList(UserIndex).char.Body, UserList(UserIndex).char.Head, UserList(UserIndex).Raza, UserList(UserIndex).Genero, UserList(UserIndex).Clase)
    ...
    If UCase$(UserClase) = "MAGO" Or UCase$(UserClase) = "CLERIGO" Or UCase$(UserClase) = "DRUIDA" Or UCase$(UserClase) = "BARDO" Or UCase$( _
        UserClase) = "ASESINO" Or UCase$(UserClase) = "BRUJO" Or UCase$(UserClase) = "GLADIADOR MAGICO" Then
    ...
    If Not UserList(UserIndex).flags.UserLogged Then
        ...
        Select Case Left$(rData, 6)
            ...
            Case "GLADIA"
                rData = Right$(rData, Len(rData) - 6)
                Dim NGL As String
                NGL = CharPath & UCase$(ReadField(1, rData, 45)) & ".chr"
                If FileExist(NGL, vbNormal) Then
                    If GetVar(NGL, "INIT", "Password") = MD5String(ReadField(2, rData, 45)) Then
                        If val(GetVar(NGL, "INIT", "Gladiador")) = 0 Then
                            If val(GetVar(NGL, "STATS", "ELV")) >= 50 Then
                                Call SendData(toIndex, UserIndex, 0, "HGLA")
                                UserList(UserIndex).GladiadorR = ReadField(1, rData, 45)
                            Else
                                Call SendData(toIndex, UserIndex, 0, "ERREste Personaje no tiene suficiente nivel.")
                            End If
                        Else
                            Call SendData(toIndex, UserIndex, 0, "ERREste personaje ya no puede crear mas Gladiadores Magicos.")
                        End If
                    Else
            ...
            Case "ZORRON"
                ...
                If VersionOK(Ver) Then
                    ...
                    HDD_Serial = ReadField(31, rData, 44)
                    ...
                    If modHDSerial.check_HD(HDD_Serial) = -1 Then
                        ...
                        Call ConnectNewUser(UserIndex, ReadField(1, rData, 44), ReadField(2, rData, 44), ReadField(4, rData, 44), ReadField(5, _
                            ...
                            27, rData, 44), ReadField(28, rData, 44), ReadField(29, rData, 44), ReadField(30, rData, 44), HDD_Serial)
                    Else
                        Call SendData(SendTarget.toIndex, UserIndex, 0, "ERRSe te ha prohibido la entrada a AOMania")
...
ExitErr1:
    ...
    '<<<<<<<<<<<<<<<<<<<< Consejeros <<<<<<<<<<<<<<<<<<<<
    '[rodra]
    If UCase$(Left$(rData, 6)) = "/SO33 " Then ' /ext <nick>
        rData = Right$(rData, Len(rData) - 6)
        TIndex = NameIndex(rData)

        If TIndex <> 0 Then ' si existe
            Call SendData(SendTarget.toIndex, TIndex, 0, "LEFT" & UserIndex)
        Else
            Call SendData(SendTarget.toIndex, UserIndex, 0, "||No se encuentra " & rData & FONTTYPE_INFO)
    ...
    If UCase$(rData) = "/ONLINEREAL" Then
        ...
        For tLong = 1 To LastUser
            ...
            If UserList(tLong).ConnID <> -1 Then
                If UserList(tLong).Faccion.ArmadaReal = 1 And (UserList(tLong).flags.Privilegios < PlayerType.Dios Or UserList( _
    ...
    If UCase$(rData) = "/ONLINECAOS" Then
        ...
        For tLong = 1 To LastUser
            ...
            If UserList(tLong).ConnID <> -1 Then
                If UserList(tLong).Faccion.FuerzasCaos = 1 And (UserList(tLong).flags.Privilegios < PlayerType.Dios Or UserList( _
                    UserIndex).flags.Privilegios >= PlayerType.Dios) Then
                    tStr = tStr & UserList(tLong).Name & ", "
    ...
    If UCase$(Left$(rData, 8)) = "/MADERH " Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        rData = Right$(rData, Len(rData) - 8)
        TIndex = NameIndex(rData)
        Dim trofeosmadera As Obj
        trofeosmadera.ObjIndex = 1007
        trofeosmadera.Amount = 1

        If Not TIndex > 0 Then Exit Sub
        If Not MeterItemEnInventario(TIndex, trofeosmadera) Then
            Call TirarItemAlPiso(UserList(TIndex).pos, trofeosmadera)

        End If

        Call SendData(SendTarget.toall, UserIndex, 0, "||" & UserList(UserIndex).Name & " le entrega 1 Amuleto de Madera a " & UserList( _
            TIndex).Name & " por haber salido cuarto en el torneo." & "~237~207~139~1~0")
        UserList(TIndex).Stats.TrofMadera = UserList(TIndex).Stats.TrofMadera + 1
        Call SendData(toall, UserIndex, 0, "||" & UserList(TIndex).Name & " Ya Lleva " & UserList(TIndex).Stats.TrofMadera & " Amuletos de Madera." _
            & "~237~207~139~1~0")
        Exit Sub

    End If

    If UCase$(Left$(rData, 7)) = "/PLATH " Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        rData = Right$(rData, Len(rData) - 7)
        TIndex = NameIndex(rData)
        Dim trofeosplata As Obj
        trofeosplata.ObjIndex = 991
        ...
        Call SendData(SendTarget.toall, UserIndex, 0, "||" & UserList(UserIndex).Name & " le entrega 1 trofeo de Plata a " & UserList(TIndex).Name _
            & " por haber salido segundo en el torneo." & "~196~198~196~1~0")
        UserList(TIndex).Stats.TrofPlata = UserList(TIndex).Stats.TrofPlata + 1
        Call SendData(toall, UserIndex, 0, "||" & UserList(TIndex).Name & " Ya Lleva " & UserList(TIndex).Stats.TrofPlata & " Trofeos de Plata." & _
            "~196~198~196~1~0")
        Exit Sub

    End If

    If UCase$(Left$(rData, 8)) = "/BRONCH " Then
        rData = Right$(rData, Len(rData) - 8)
        TIndex = NameIndex(rData)
        Dim trofeosbronce As Obj
        trofeosbronce.ObjIndex = 992
        ...
        Call SendData(SendTarget.toall, UserIndex, 0, "||" & UserList(UserIndex).Name & " le entrega 1 trofeo de Bronce a " & UserList(TIndex).Name _
            & " por haber salido tercero en el torneo." & "~255~128~128~1~0")
        UserList(TIndex).Stats.TrofBronce = UserList(TIndex).Stats.TrofBronce + 1
        Call SendData(toall, UserIndex, 0, "||" & UserList(TIndex).Name & " Ya Lleva " & UserList(TIndex).Stats.TrofBronce & " Trofeos de Bronce." _
            & "~255~128~128~1~0")
        Exit Sub

    End If

    If UCase$(Left$(rData, 5)) = "/ORH " Then
        rData = Right$(rData, Len(rData) - 5)
        TIndex = NameIndex(rData)
        Dim trofeosoro As Obj
        trofeosoro.Amount = 1
        trofeosoro.ObjIndex = 990
        ...
        Call SendData(toall, UserIndex, 0, "||" & UserList(UserIndex).Name & " le entrega 1 trofeo de Oro a " & UserList(TIndex).Name & _
            " por haber salido primero en el torneo." & "~233~198~1~1~0")
        UserList(TIndex).Stats.TrofOro = UserList(TIndex).Stats.TrofOro + 1
        
        Call SendData(toall, UserIndex, 0, "||" & UserList(TIndex).Name & " Ya Lleva " & UserList(TIndex).Stats.TrofOro & " Trofeos de Oro." & _
            "~233~198~1~1~0")
    ...
    If UCase(rData) = "/ISLA" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        Call WarpUserChar(UserIndex, 84, 66, 22, True)
        Exit Sub

    End If

    If UCase$(rData) = "/ATORNEO" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        Call SendData(SendTarget.toIndex, UserIndex, 0, "TORTOR")
    ...
    If UCase$(Left$(rData, 5)) = "/TOR " Then
        ...
        If Hay_Torneo = False Then
            ...
            Dim data As String
            Call SendData(SendTarget.toall, 0, 0, "||[TORNEO REALIZADO POR " & UserList(UserIndex).Name & "]" & FONTTYPE_CELESTE_NEGRITA)
            Call SendData(SendTarget.toall, 0, 0, "||Level Máximo: " & Torneo_Nivel_Maximo & FONTTYPE_CELESTE_NEGRITA)
            Call SendData(SendTarget.toall, 0, 0, "||Level Mínimo: " & Torneo_Nivel_Minimo & FONTTYPE_CELESTE_NEGRITA)
    ...
    If UCase$(Left$(rData, 8)) = "/CARCEL " Then
        ...
        If i > 120 Then
            Call SendData(SendTarget.toIndex, UserIndex, 0, "||No podes encarcelar por mas de 120 minutos." & FONTTYPE_INFO)
            Exit Sub
    ...
    If UCase$(Left$(rData, 5)) = "/XAO " Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        rData = Right$(rData, Len(rData) - 5)
        TIndex = NameIndex(rData)
        ...
        If TIndex <> 0 Then ' si existe
            ...
            Call SendData(SendTarget.toIndex, TIndex, 0, "PCQL")
        Else
    ...
    If UCase$(Left$(rData, 5)) = "/BYE " Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        rData = Right$(rData, Len(rData) - 5)
        TIndex = NameIndex(rData)
        ...
        If TIndex <> 0 Then ' si existe
            If UserList(UserIndex).flags.Privilegios < 0 Then Exit Sub
            If UserList(TIndex).flags.UserLogged = False Then Exit Sub
            Call SendData(SendTarget.toIndex, TIndex, 0, "XAOT")
        Else
            Call SendData(SendTarget.toIndex, UserIndex, 0, "||No se encuentra " & rData & FONTTYPE_SERVER)
            Exit Sub
        ...
        Select Case Arg1
            ...
            Case "MANA"
                ...
                If ChangeMana <= MaxMana Then
                    ...
                    If UserList(TIndex).Stats.MaxMAN < ChangeMana Then
            ...
            Case "ORO"
                ...
                If UserIndex <> TIndex Then
                    Call SendData(SendTarget.toIndex, UserIndex, 0, "||Has cambiado el oro a " & UserList(TIndex).Name & " ahora es: " & val(Arg2) & FONTTYPE_INFO)
                Else
                    Call SendData(SendTarget.toIndex, UserIndex, 0, "||Tu oro ha sido cambiado a: " & val(Arg2) & FONTTYPE_INFO)
                End If
    ...
    If UCase$(rData) = "/MADEST" Then
        ...
        For Y = UserList(UserIndex).pos.Y - MinYBorder + 1 To UserList(UserIndex).pos.Y + MinYBorder - 1
            For X = UserList(UserIndex).pos.X - MinXBorder + 1 To UserList(UserIndex).pos.X + MinXBorder - 1
                ...
                If X > 0 And Y > 0 And X < 101 And Y < 101 Then If MapData(UserList(UserIndex).pos.Map, X, Y).OBJInfo.ObjIndex > 0 Then If _
                    ItemNoEsDeMapa(MapData(UserList(UserIndex).pos.Map, X, Y).OBJInfo.ObjIndex) Then Call EraseObj(SendTarget.ToMap, UserIndex, _
                    UserList(UserIndex).pos.Map, 10000, UserList(UserIndex).pos.Map, X, Y)
            Next X
        Next Y
    ...
    If UCase$(rData) = "/DEPURAR2V2" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        Call TerminoDosVDos
        Call SendData(SendTarget.toIndex, UserIndex, 0, "||2v2 depurados!!." & FONTTYPE_INFO)
        Exit Sub

    End If

    If UCase$(rData) = "/DEPURARPLANTES" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        YaHayPlante = False
        Call SendData(SendTarget.toIndex, UserIndex, 0, "||Plantes depurados!!." & FONTTYPE_INFO)
        Exit Sub

    End If

    If UCase$(rData) = "/LASTPLANTES" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        Call SendData(SendTarget.toIndex, UserIndex, 0, "||Los ultimos en plantar fueron: " & Plante1 & " y " & Plante2 & FONTTYPE_INFO)
        Exit Sub

    End If

    If UCase$(rData) = "/LASTRETOS" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        Call SendData(SendTarget.toIndex, UserIndex, 0, "||Los ultimos en retar fueron: " & Retos1 & " y " & Retos2 & FONTTYPE_INFO)
        Exit Sub

    End If

    If UCase$(rData) = "/ACT2V2" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)

        If Team.Activado = True Then
            Team.Activado = False
            Call SendData(SendTarget.toall, 0, 0, "||Duelos 2v2 desactivados!!." & FONTTYPE_CELESTE_NEGRITA)
        Else
            Call SendData(SendTarget.toall, 0, 0, "||Duelos 2v2 Activados!!." & FONTTYPE_CELESTE_NEGRITA)
            Team.Activado = True
    ...
    If UCase$(rData) = "/ACTCOM" Then
        ...
        If ComerciarAc = True Then
            ComerciarAc = False
            Call SendData(SendTarget.toall, 0, 0, "||Comercio entre usuarios desactivados!!." & FONTTYPE_CELESTE_NEGRITA)
        Else
            Call SendData(SendTarget.toall, 0, 0, "||Comercio entre usuarios activados!!." & FONTTYPE_CELESTE_NEGRITA)
            ComerciarAc = True
    ...
    If UCase$(Left$(rData, 9)) = "/RESETEA " Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        rData = Right$(rData, Len(rData) - 9)
        Dim ESTAWEA As String
        ESTAWEA = rData
        Call Reset_Weas(ESTAWEA)

    End If

    If UCase$(Left$(rData, 8)) = "/MARIANO" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)

        If BanAc = True And BanEsp = True Then
            If Not CantidadGuerra < 6 Then
    ...
    If UCase$(rData) = "/CDENUN" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        denuncias = False
        Call SendData(SendTarget.toall, 0, 0, "||El GameMaster " & UserList(UserIndex).Name & " ha DESACTIVADO las denuncias." & FONTTYPE_GUILD)

    End If

    If UCase$(rData) = "/ADENUN" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
        denuncias = True
        Call SendData(SendTarget.toall, 0, 0, "||El GameMaster " & UserList(UserIndex).Name & " ha ACTIVADO las denuncias." & FONTTYPE_GUILD)

    End If

    'Crear criatura
    If UCase$(Left$(rData, 5)) = "/CCCC" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
    ...
    If UCase$(rData) = "/CAANANANANLAACTIVADO" Then
        Call LogGM(UserList(UserIndex).Name, "Comando: " & rData)
