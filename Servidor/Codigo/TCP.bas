...
Sub ConnectNewUser(UserIndex As Integer, _
    ...
    UserList(UserIndex).Ignore.MaximoIgnores = 3
...
Sub Proceso_Desconexion(ByVal UserIndex As Integer)
    ...
    error = 17
     ...
     If NameDay = "Noche" And UCase$(UserList(UserIndex).Raza) = "LICANTROPO" Then
         Call DesconectaPoderLicantropo(UserIndex)
     End If
                ...
                If InMapBounds(sndMap, X, Y) Then
                    If MapData(sndMap, X, Y).UserIndex > 0 Then
                        If UserList(MapData(sndMap, X, Y).UserIndex).ConnID <> -1 Then
                          ...
                          If UserList(sndIndex).flags.HablanMute = 1 Then
                               If Not EsUserMute(sndIndex, MapData(sndMap, X, Y).UserIndex) Then
