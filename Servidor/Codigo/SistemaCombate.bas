...
Function PoderEvasion(ByVal UserIndex As Integer) As Long
    ...
    With UserList(UserIndex)
        lTemp = (.Stats.UserSkills(eSkill.Tacticas) + .Stats.UserSkills(eSkill.Tacticas) / 33 * .Stats.UserAtributos(eAtributos.Agilidad)) * ModClase(ClaseToByte(.Clase)).Evasion
...
Public Sub UserDañoNpc(ByVal UserIndex As Integer, ByVal NpcIndex As Integer)
    ...
    Call SendData(SendTarget.ToPCArea, UserIndex, UserList(UserIndex).Pos.Map, "||" & vbCyan & "°" & Daño & "°" & CStr(Npclist(NpcIndex).char.CharIndex))
    ...
    If Npclist(NpcIndex).Stats.MinHP <= 0 Then
        ...
        Call SendData(SendTarget.ToIndex, UserIndex, 0, "||" & Npclist(NpcIndex).Name & " le queda " & Npclist(NpcIndex).Stats.MinHP & " / " & Npclist(NpcIndex).Stats.MaxHP & " de vida." & FONTTYPE_Motd4)
    ...
    If IntervaloPermiteAtacar(UserIndex) Then
        ...
        If MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex > 0 Then
            ...
            If Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).Attackable Then
                ...
                If Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).MaestroUser > 0 And MapInfo(Npclist(MapData(AttackPos.Map, AttackPos.X, AttackPos.Y).NpcIndex).Pos.Map).Pk = False Then
...
Public Sub UserDañoUser(ByVal AtacanteIndex As Integer, ByVal VictimaIndex As Integer)
    ...
    If Daño >= 200 Then
        If UserList(VictimaIndex).flags.Navegando = 0 Then Call SendData(SendTarget.ToPCArea, VictimaIndex, UserList(VictimaIndex).Pos.Map, "CFX" & UserList(VictimaIndex).char.CharIndex & "," & 38 & "," & 0)

    End If

    If Daño < 200 Then
        If UserList(VictimaIndex).flags.Navegando = 0 Then Call SendData(SendTarget.ToPCArea, VictimaIndex, UserList(VictimaIndex).Pos.Map, "CFX" & UserList(VictimaIndex).char.CharIndex & "," & 14 & "," & 0)
...
Public Function ZonaDuelos(ByVal Map As Integer, _
...
Public Function ProhibidoAtacar(ByVal Map As Integer, _
                                ByVal X As Integer, _
                                ByVal Y As Integer) As Boolean
...
Public Function PuedeAtacar(ByVal AttackerIndex As Integer, _
    ...
    If Not UserList(AttackerIndex).flags.SeguroCombate Then
        ...
        Call SendData(SendTarget.ToIndex, AttackerIndex, 0, "||No estás en modo de combate, presiona la tecla ""C"" para pasar al modo combate." & FONTTYPE_Motd4)
        PuedeAtacar = False
        Exit Function
...
Public Function TriggerZonaPelea(ByVal Origen As Integer, _
    ...
    If Origen > 0 And Destino > 0 And Origen <= UBound(UserList) And Destino <= UBound(UserList) Then
        If MapData(UserList(Origen).Pos.Map, UserList(Origen).Pos.X, UserList(Origen).Pos.Y).Trigger = eTrigger.ZONAPELEA Or MapData(UserList(Destino).Pos.Map, UserList(Destino).Pos.X, UserList(Destino).Pos.Y).Trigger = eTrigger.ZONAPELEA Then
